...
- Webhook SDK registry publish readiness API with npm/PyPI owner approval gates, provenance/2FA/README/smoke evidence checks, CSV export, OpenAPI/link-health/product-map coverage, and owner-dependent registry parking-lot notes. Completed.
...
- Partner live-smoke execution readiness API with owner approval metadata, production key custody reference checks, write-path sandbox-only guardrails, archive-owner evidence, CSV export, OpenAPI/link-health/product-map coverage, and live-key parking-lot updates. Completed.
...
- Partner cursor pagination production-volume evidence API with endpoint thresholds, owner-provided row/sync/page-depth env gates, CSV export, OpenAPI/link-health/product-map coverage, and owner-dependent volume parking-lot notes. Completed.
